    ...
    $romaneio = '';
        ...
        foreach ($box_option as $idpedido) {
            try {
                ...
                $monitor = 'T';
                $sql = $pdo->prepare("SELECT produto.descricao,produto.valor_venda,SUM(produto_no_pedido.quantidade) AS quantidade,SUM(produto_no_pedido.subtotal) AS subtotal FROM produto INNER JOIN produto_no_pedido ON produto_no_pedido.produto_idproduto = produto.idproduto INNER JOIN pedido ON produto_no_pedido.pedido_idpedido = pedido.idpedido INNER JOIN vendedor ON pedido.vendedor_idvendedor = vendedor.idvendedor WHERE pedido.monitor = :monitor AND pedido.idpedido IN (:idpedido) AND vendedor.idvendedor = :idvendedor GROUP BY produto.descricao ORDER BY produto.descricao");
