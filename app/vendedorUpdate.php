<?php
    ...
    $lock = base64_encode('cripta');
        ...
        if(empty($_POST['idvendedor'])) { die("Vari&aacute;vel de controle nula."); }
        ...
        if($filtro == 5) {
            try {
                ...
                $sql = $pdo->prepare("SELECT idvendedor,monitor FROM vendedor WHERE email = :email AND idvendedor <> :idvendedor");
                $sql->bindParam(':email', $_POST['email'], PDO::PARAM_STR);
                $sql->bindParam(':idvendedor', $_POST['idvendedor'], PDO::PARAM_INT);
                $sql->execute();
                $ret = $sql->rowCount();
                ...
                $sql = $pdo->prepare("UPDATE vendedor SET tipo = :tipo,nome = :nome,usuario = :usuario,senha = :senha,email = :email WHERE idvendedor = :idvendedor");
                ...
                $res = $sql->execute();
                    ...
                    if(!$res) {
                        ...
                        $log_hora = date('H:i:s');
                        $log_descricao = 'Usuário '.$_SESSION['seller'].' atualizou o cadastro do vendedor '.$_POST['nome'].'.';
