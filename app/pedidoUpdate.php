    ...
    $msg = "Campo obrigat&oacute;rio vazio.";
        ...
        if($filtro == 5) {
            try {
                ...
                $sql = $pdo->prepare("SELECT idpedido,monitor FROM pedido WHERE codigo = :codigo AND idpedido <> :idpedido");
                ...
                $sql = $pdo->prepare("UPDATE pedido SET vendedor_idvendedor = :vendedor,cliente_idcliente = :cliente,tipo = :tipo,datado = :datado,hora = :hora WHERE idpedido = :idpedido");
                ...
                $res = $sql->execute();
                    ...
                    if(!$res) {
                        ...
                        $log_hora = date('H:i:s');
                        $log_descricao = 'Usuário '.$_SESSION['seller'].' atualizou um orçamento/pedido.';
