<?php
    ...
    $msg = "Campo obrigat&oacute;rio vazio.";
        ...
        if(empty($_POST['idpraca'])) { die("Vari&aacute;vel de controle nula."); }
        ...
        if($filtro == 2) {
            try {
                ...
                $sql = $pdo->prepare("SELECT idpraca,monitor FROM praca WHERE vendedor_idvendedor = :idvendedor AND nome = :nome AND idpraca <> :idpraca");
                $sql->bindParam(':idvendedor', $_POST['vendedor'], PDO::PARAM_INT);
                $sql->bindParam(':nome', $_POST['nome'], PDO::PARAM_STR);
                $sql->bindParam(':idpraca', $_POST['idpraca'], PDO::PARAM_INT);
                $sql->execute();
                $ret = $sql->rowCount();
                ...
                $sql = $pdo->prepare("UPDATE praca SET vendedor_idvendedor = :idvendedor,nome = :nome WHERE idpraca = :idpraca");
                $sql->bindParam(':idvendedor', $_POST['vendedor'], PDO::PARAM_INT);
                $sql->bindParam(':nome', $_POST['nome'], PDO::PARAM_STR);
                $sql->bindParam(':idpraca', $_POST['idpraca'], PDO::PARAM_INT);
                $res = $sql->execute();
                    ...
                    if(!$res) {
                        ...
                        $log_hora = date('H:i:s');
                        $log_descricao = 'Usuário '.$_SESSION['seller'].' atualizou uma praça.';
