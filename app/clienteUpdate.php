    ...
    $msg = "Campo obrigat&oacute;rio vazio.";
        ...
        if($filtro == 3) {
            try {
                ...
                $sql = $pdo->prepare("SELECT idcliente,monitor FROM cliente WHERE cpf_cnpj = :cpf_cnpj AND idcliente <> :idcliente");
                ...
                $sql = $pdo->prepare("UPDATE cliente SET praca_idpraca = :praca,nome = :nome,cpf_cnpj = :cpf_cnpj,cep = :cep,endereco = :endereco,bairro = :bairro,cidade = :cidade,estado = :estado,telefone = :telefone,celular = :celular,email = :email WHERE idcliente = :idcliente");
                ...
                $res = $sql->execute();
                    ...
                    if(!$res) {
                        ...
                        $log_hora = date('H:i:s');
                        $log_descricao = 'Usuário '.$_SESSION['seller'].' atualizou o cadastro do cliente '.$_POST['nome'].'.';
